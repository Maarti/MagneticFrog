...
public class LevelSettings {
    ...
    public readonly static Dictionary<float, LevelSettings> levelSettings = new Dictionary<float, LevelSettings> {
            { LEVEL_10_SCORE, new LevelSettings(LEVEL_10_SCORE, LEVEL_20_SCORE, 15f,16f,    .5f,2f,    -1f,-1f,    -1f,-1f) },
            { LEVEL_20_SCORE, new LevelSettings(LEVEL_20_SCORE, LEVEL_30_SCORE, 8f,12f,     1.5f,3f,   2f,4f,      .5f,1f) },
            { LEVEL_30_SCORE, new LevelSettings(LEVEL_30_SCORE, LEVEL_40_SCORE, 6f, 10f,    2f, 4f,    1f,3f,      .75f,2f) },
        };
    ...
    public LevelSettings(float scoreMin, float scoreMax, float coinMinWait, float coinMaxWait, float bubbleMinWait, 
        float bubbleMaxWait, float mineMinWait, float mineMaxWait, float mineMinSize, float mineMaxSize) {
